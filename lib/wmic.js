...
function wmic (pids, options, done) {
  ...
  var columns = 'CreationDate,KernelModeTime,ParentProcessId,PrivatePageCount,ProcessId,UserModeTime,WorkingSetSize'.split(',');
  var args = [
    'PROCESS',
    'where',
    '"' + whereClause + '"',
  ...
  bin('wmic', args, {windowsHide: true, windowsVerbatimArguments: true}, function (err, stdout, code) {
    ...
    var again = false
    var statistics = {}
    var map = stdout[0].trim().split(/\s+/).reduce((s, v, i) => {
      s[v] = i;
      return s;
    }, {});
    for (var i = 1; i < stdout.length; i++) {
      ...
      if (!line || line.length !== columns.length) {
        continue
      }

      var creation = parseDate(line[map['CreationDate']])
      var ppid = parseInt(line[map['ParentProcessId']], 10)
      var pid = parseInt(line[map['ProcessId']], 10)
      var kerneltime = Math.round(parseInt(line[map['KernelModeTime']], 10) / 10000)
      var usertime = Math.round(parseInt(line[map['UserModeTime']], 10) / 10000)
      var memory = parseInt(line[map[options.private ? 'PrivatePageCount' : 'WorkingSetSize']], 10)
